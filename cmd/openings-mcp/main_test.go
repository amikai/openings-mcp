...
func TestServerListsJobTools(t *testing.T) {
	ctx := t.Context()
	cAmazon, err := amazon.NewClient(amazon.DefaultBaseURL, amazon.WithClient(http.DefaultClient))
	require.NoError(t, err)
	c104, err := job104.NewClient(job104.DefaultBaseURL, job104.WithClient(http.DefaultClient))
	require.NoError(t, err)
	cApple, err := apple.NewJobsClient(apple.DefaultBaseURL, http.DefaultClient)
	require.NoError(t, err)
	cCake, err := cake.NewClient(cake.DefaultBaseURL, cake.WithClient(http.DefaultClient))
	require.NoError(t, err)
	cNvidia, err := nvidia.NewClient(nvidia.DefaultBaseURL, nvidia.WithClient(http.DefaultClient))
	require.NoError(t, err)
	cTsmc := tsmc.NewClient(tsmc.DefaultBaseURL, http.DefaultClient)
	cGoogle := google.NewClient(google.DefaultBaseURL, http.DefaultClient)
	cLinkedin := linkedin.NewClient(linkedin.DefaultBaseURL, http.DefaultClient)
	cIndeed := indeed.NewClient(indeed.DefaultBaseURL, http.DefaultClient)
	cJobindex := jobindex.NewClient(jobindex.DefaultBaseURL, http.DefaultClient)
