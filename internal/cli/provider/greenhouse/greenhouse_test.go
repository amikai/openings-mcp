...
func TestRunSearchMissingBoard(t *testing.T) {
	err := runSearch(t.Context(), searchOptions{timeout: time.Second, format: "text"})
	assert.ErrorContains(t, err, "--board is required")
}

func TestRunSearchUnknownBoard(t *testing.T) {
	err := runSearch(t.Context(), searchOptions{board: "doesnotexist-board-xyz", timeout: time.Second, format: "text"})
...
func TestRunGetMissingID(t *testing.T) {
	err := runGet(t.Context(), getOptions{board: "anthropic", timeout: time.Second, format: "text"})
	assert.ErrorContains(t, err, "--id is required")
}

func TestRunGetMissingBoard(t *testing.T) {
	err := runGet(t.Context(), getOptions{timeout: time.Second, jobID: 123, format: "text"})
	assert.ErrorContains(t, err, "--board is required")
}

func TestRunGetUnknownBoard(t *testing.T) {
	err := runGet(t.Context(), getOptions{board: "doesnotexist-board-xyz", timeout: time.Second, jobID: 123, format: "text"})
	require.ErrorContains(t, err, `board "doesnotexist-board-xyz" not found`)
	assert.ErrorContains(t, err, "greenhouse companies")
}
