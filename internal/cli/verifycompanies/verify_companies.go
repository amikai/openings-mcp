// Package verifycompanies verifies every curated companies.yaml entry by
...
// NewCommand returns a cobra.Command for verify-companies.
func NewCommand() *cobra.Command {
	opts := &options{}

	cmd := &cobra.Command{
		Use:          "verify-companies",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if errs > 0 {
				return fmt.Errorf("verification failed: %d company check(s) failed", errs)
			}
			return nil
		},
	}
