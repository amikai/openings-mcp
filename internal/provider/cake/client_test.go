...
func newMockServer(t *testing.T) *httptest.Server {
	...
	mux.HandleFunc("/api/client/v1/jobs/search", func(w http.ResponseWriter, r *http.Request) {
...
func TestSearchJobs(t *testing.T) {
	...
	require.True(t, ok)
	assert.Equal(t, "lctech_", page.Path.Value)
	for i := range got.Data {
		got.Data[i].Page.Reset()
	}

	assert.Equal(t, &JobSearchResponse{
		TotalEntries: NewNilInt(360),
		TotalPages:   NewNilInt(18),
		PerPage:      NewNilInt(20),
		CurrentPage:  NewNilInt(1),
		Data: []JobSearchItem{
			{
				Path:  "senior-golang-web-backend-engineer-taoyuan",
				Title: NewNilString("資深 Golang 網頁後端工程師(桃園)"),
				Description: NewNilString(`【職務內容】
...
不定時員工聚餐與每年都有員工旅遊。`),
			},
			{
				...
				Title:       NewNilString("Senior Backend Golang Developer / 資深後端 Golang 工程師"),
				Description: NewNilString(`▎What You'll Do身為資深 Golang 後端工程師，你將參與支援百萬級用戶、高 TPS 的大型系統設計與開發，聚焦於即時通訊、社群互動、支付與平台功能模組，並攜手跨部門夥伴共同交付高品質產品。你的實際工作將包含：1. 建構與優化高併發、高穩定性、高可維護性的後端架構。2. 負責即時訊息、推播、社群互動、錢包支付等模組開發與持續演進。3. 深入參與系統資源調度與效能優化策略設計。4. 對問題進行深入剖析，提出具邏輯性與可落地的解法。5. 與產品、前端、UI/UX、測試團隊密切協作，從需求釐清到上線監控全流程參與。6. 評估技術選型，理解架構決策背後的 trade-off，打造可持續演進的系統。`),
			},
			{
				Path:  "4672975006-solution-engineer-c-golang-python-9f5e80aaa20eac33b0662a03591d00",
				Title: NewNilString("Solution Engineer (C++ / Golang / Python)"),
				Description: NewNilString(`WorldQuant develops and deploys systematic financial strategies across a broad range of asset classes and global markets. We seek to produce high-quality predictive signals (alphas) through our proprietary research platform to employ financial strategies focused on market inefficiencies. Our teams work collaboratively to drive the production of alphas and financial strategies – the foundation of a balanced, global investment platform.
...
Copyright © 2025 WorldQuant, LLC. All Rights Reserved.WorldQuant is an equal opportunity employer and does not discriminate in hiring on the basis of race, color, creed, religion, sex, sexual orientation or preference, age, marital status, citizenship, national origin, disability, military status, genetic predisposition or carrier status, or any other protected characteristic as established by applicable law.`),
			},
			{
				...
				Title:       NewNilString("Senior Backend Developer / 資深後端工程師 ( 轉 Golang )"),
			...
			},
			{
				...
				Title: NewNilString("【純遠端 T】跨國遊戲公司 誠徵 Go工程師"),
				Description: NewNilString(`1、負責GRPC和大廳業務；
...
6、負責公司後端架構的搭建和優化，持續優化服務的可用性、伸縮性、穩定性；`),
			},
			{
				...
				Title: NewNilString("Software Engineer(Golang, Flutter), Virtual insurance"),
				Description: NewNilString(`About the Role
...
`),
			},
			{
				...
				Title: NewNilString("Senior Developer 紅隊工具開發工程師"),
				Description: NewNilString(`你將與台灣最頂尖的資安專家並肩作戰，負責開發及維護紅隊演練工具。你參與的專案幾乎是外面不會有的需求，適合熱愛技術挑戰的你。你可以直接面對使用者討論，通常使用者也能直接給予技術回饋甚至方案建議。團隊成員對自己的作品充滿認同與自豪，若你也熱衷技術、追求卓越、重視團隊，歡迎加入我們！下面是工作內容：
...
基本資料學歷工作經歷MBTI 職業性格測試結果（測試網頁）若您願意提供 MBTI 測驗結果，可讓我們更瞭解您偏好的溝通模式，若您不願意提供，也不影響本次審核結果。`),
			},
			{
				...
				Title: NewNilString("後端工程師 (Golang/Go)"),
				Description: NewNilString(`負責核心後端服務與微服務架構的設計、開發與優化使用 Go（Golang）打造高效能、高可用且具擴展性的系統深度參與系統架構演進，支撐產品在高併發與大規模使用情境下穩定運行與產品、前端及 DevOps 團隊協作，確保服務品質與交付效率如果你熱衷於打造高效能分散式系統，並希望在微服務與雲端架構中發揮技術影響力，歡迎加入我們！主要職責端開發與系統架構
...
熟悉雲端環境（AWS、GCP）服務運作`),
			},
			{
				...
				Title: NewNilString("Senior Backend Engineer (Golang)（每月有遠端日）"),
				Description: NewNilString(`團隊介紹
...
`),
			},
			{
				...
				Title: NewNilString("🌍 薪資福利優渥 - Sr. Backend Engineer(Golang, Node.js, Java) to Tech Lead｜AI 應用核心系統開發 - Candice"),
				Description: NewNilString(`我們正在尋找一位想主導架構、影響產品方向、打造可長期演進的系統的優秀人才設計與優化 高可用、高效能、可擴展 的後端系統架構 
...
`),
			...
			{
				Path:  "gip",
				Title: NewNilString("Software Engineer Intern (Onsite; Tokyo)"),
				Description: NewNilString(`About the job
...
`),
			},
			{
				...
				Title:       NewNilString("後端工程師｜Backend Engineer（區塊鏈 Web3 公司）"),
				Description: NewNilString(`協助開發 Web3 安全相關產品研究新技術，導入並優化現有產品集思廣益，讓產品更好依照需求開發後端相關功能`),
			},
			{
				Path:  "832c3d",
				Title: NewNilString("Golang 工程師 (台中)"),
				Description: NewNilString(`▌工作內容
...
技術文件建置：負責架構與開發流程的文件化，透過完整的文件撰寫，讓團隊知識得以傳承，並降低溝通成本。`),
			},
			{
				...
				Title: NewNilString("💼【高併發 Fintech 系統】Sr. Backend Engineer（Java／歡迎 Golang、C# 轉換）-TL"),
				Description: NewNilString(`
...
`),
			},
			{
				...
				Title: NewNilString("Senior Backend Engineer (Golang), Virtual insurance"),
				Description: NewNilString(`Responsibilities
Design, build, and maintain backend services for B2C Insurance/IoT/Shopping/Point/Payment systems.Build the APIs and design the data schema to fulfill the business requirements.Write tests and constantly seek to improve code quality and reliability.Drive the quality standards within the development team by example, produce highly usable technical documentation as well as conduct code reviews.Collaborate with cross-functional teams to deliver end-to-end solutions.`),
			},
			{
				...
				Title:       NewNilString("資深Golang工程師"),
				Description: NewNilString(`■ 薪資條件：60,000~200,000 NTD■ 工作時間：9:00~18:00需求分析與後端架構設計及開發：依據產品需求，使用Golang語言設計和改進系統架構，確保系統的可擴展性、穩定性和性能API Server的開發與串接：設計和實現RESTful，並確保API的可靠性和安全性性能優化：不斷優化程式碼效能、實現高品質且可重複利用的程式碼，並提升系統的響應速度和處理能力問題排查：處理各類軟體技術問題與 Bug 修正跨部門協作：參與跨部門合作，與團隊成員協作配合，共同開發和解決技術問題件`),
			},
		},
	}, got)
}
...
func TestGetJobDetail(t *testing.T) {
	...
	assert.Equal(t, &JobDetail{
		...
		Title:    NewNilString("資深 Golang 網頁後端工程師(桃園)"),
		Description: NewNilString(`<p>【職務內容】</p>
...
<p>不定時員工聚餐與每年都有員工旅遊。</p>`),
		Requirements: NewNilString(`<p><strong></strong><strong>【基本條件】</strong></p>
<p>※ 應徵者務必附上作品或相關連結</p>
